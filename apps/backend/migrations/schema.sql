...
-- Enterprise-grade Data Lineage and PII Classification Platform
...
CREATE TABLE source_profiles (
    ...
    description TEXT,
    data_source_type VARCHAR(100) NOT NULL, -- fs, db, s3, etc.
    configuration JSONB,
...
CREATE TABLE assets (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    stable_id VARCHAR(255) UNIQUE NOT NULL, -- Hash-based stable identifier
    asset_type VARCHAR(100) NOT NULL, -- file, database, table, column, etc.
    name VARCHAR(500) NOT NULL,
    path TEXT NOT NULL,
    data_source VARCHAR(100) NOT NULL,
    host VARCHAR(255),
    file_metadata JSONB, -- filename, extension, size, permissions, etc.
...
CREATE TABLE patterns (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name VARCHAR(255) UNIQUE NOT NULL,
    pattern_type VARCHAR(100) NOT NULL, -- regex, keyword, ml, etc.
    category VARCHAR(100) NOT NULL, -- pii, secret, sensitive, etc.
    description TEXT,
...
CREATE TABLE findings (
    ...
    matches TEXT[], -- Array of matched values
    sample_text TEXT,
...
CREATE TABLE classifications (
    ...
    classification_type VARCHAR(100) NOT NULL, -- Personal Data, Sensitive Personal Data, Secrets, Non-PII
    sub_category VARCHAR(100), -- Email, Phone, Credit Card, API Key, etc.
    confidence_score DECIMAL(5,2) NOT NULL, -- 0.00 to 100.00
    justification TEXT,
    dpdpa_category VARCHAR(100), -- DPDPA compliance category
...
CREATE TABLE asset_relationships (
    ...
    relationship_type VARCHAR(100) NOT NULL, -- CONTAINS, EXPOSES, CLASSIFIED_AS, REFERENCES, etc.
    metadata JSONB,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE review_states (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    finding_id UUID NOT NULL REFERENCES findings(id) ON DELETE CASCADE,
    status VARCHAR(50) NOT NULL DEFAULT 'pending', -- pending, reviewing, approved, rejected, false_positive
